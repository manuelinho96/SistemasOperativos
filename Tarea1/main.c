...
int main (){
	...
	char *nom;
	struct Producto *p1;
	if ((p1 = malloc (sizeof (struct Producto))) == NULL) return -1;
	if ((tas = malloc (sizeof (Stack))) == NULL) return -1;
	if ((nom = malloc (50 * sizeof (char))) == NULL) return -1;
	initialization(tas);
	...
	scanf ("%s", nom);
	struct Producto *p2;
	if ((p2 = malloc (sizeof (struct Producto))) == NULL) return -1;
	...
	scanf ("%s", nom);
	struct Producto *p3;
	if ((p3 = malloc (sizeof (struct Producto))) == NULL) return -1;
